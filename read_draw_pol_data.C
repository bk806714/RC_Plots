...
gStyle->SetTitleXSize(0.055);
 ...
 leakage_factorized(lab,kine);
 
  gr_Bi[0]->Draw("AL");
  gr_Bi[0]->GetXaxis()->SetTitle("Ph_{T} (GeV/c)");
  gr_Bi[0]->GetYaxis()->SetTitle("#int #int sin(^{}#phi_{h} + #phi_{S})d#sigma ");
  gr_Bi[0]->SetLineStyle(2);
  gr_Bi[0]->GetYaxis()->SetRangeUser(-0.01e-6,1e-6);
  gr_Bi[0]->GetXaxis()->SetRangeUser(0,1.3);
  gr_Bi[1]->SetLineColor(2);
  ...
  gr_F_SC_R->Draw("LSAME");
  gr_F_SC_R->SetLineColor(3);
  gr_F_SC_R->SetLineStyle(9);


  gr_F_SC_B->SetLineColor(3);
  gr_F_SC_B->SetLineStyle(2);
  gr_F_SC_B->SetLineWidth(5);
  gr_F_SC_B->Draw("LSAME");
